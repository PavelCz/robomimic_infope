...
class BCConfig(BaseConfig):
    ...
    def algo_config(self):
        ...
        self.algo.rnn.open_loop = (
            False  # if True, action predictions are only based on a single observation (not sequence)
        )
        ...
        self.algo.transformer.context_length = (
            10  # length of (s, a) seqeunces to feed to transformer - should usually match train.frame_stack
        )
        ...
        self.algo.transformer.num_heads = (
            8  # number of attention heads for each transformer block (should divide embed_dim evenly)
        )
        ...
        self.algo.transformer.pred_future_acs = (
            False  # shift action prediction forward to predict future actions instead of past actions
        )
