...
def train(config, device, resume=False, disable_wandb=False):
    ...
    if config.experiment.logging.terminal_output_to_txt:
        ...
        logger = LogUtils.PrintLogger(os.path.join(log_dir, "log.txt"))
        sys.stdout = logger
        sys.stderr = logger
    ...
    # * load training data
    ope_kwargs = dict(dinov3_embedding_path=None, target_actions_paths=None)
    ...
    data_logger = LogUtils.DataLogger(
        log_dir,
        config,
...
def main(args):
    ...
    # .../robomimic0.5/lift/mh/image_v15.hdf5 -> lift/mh/image_v15 -> lift-mh-image_v15
    ds_name = args.dataset.split("robomimic0.5/")[1].split(".hdf5")[0]
    ds_name = ds_name.replace("/", "-")
    exp_name = f"{config.algo_name}_{ds_name}_Ta={config.algo.horizon.action_horizon}"
    exp_name = f"{time_str}_{exp_name}"
    config.experiment.name = exp_name if args.name is None else args.name
